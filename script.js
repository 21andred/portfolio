...
function creaProgetto(progetto, indice) {
  ...
  const reverse = indice % 2 === 1 ? ' progetto-reverse' : '';
  const tag = (progetto.tag || []).map((t) => `<li>${t}</li>`).join('');

  return `
    <article class="progetto${reverse}" id="progetto-${progetto.id}">
      ${creaVisual(progetto)}
...
function renderizzaProgetti() {
  ...
  container.innerHTML = PROGETTI.map((progetto, indice) => creaProgetto(progetto, indice)).join('');
